...
int kernel_main(void)
{
  ...
  argvector = read_command_line();
  argvector = parse_options(argvector);
...
static void paging_init(void)
{
  uint32_t *pagedirectoryy;
  uint32_t *pt;
  size_t pagesiz;
  extern char _start, _end_kernel_text;

  pagedirectoryy = init_page_dir = palloc_get_page(PAL_ASSERT | PAL_ZERO);
  pt = NULL;
  for (pagesiz = 0; pagesiz < init_ram_pages; pagesiz++)
  {
    uintptr_t paddr = pagesiz * PGSIZE;
    char *vaddr = ptov(paddr);
    size_t pde_idx = pd_no(vaddr);
    size_t pte_idx = pt_no(vaddr);
    bool in_kernel_text = &_start <= vaddr && vaddr < &_end_kernel_text;

    if (pagedirectoryy[pde_idx] == 0)
    {
      pt = palloc_get_page(PAL_ASSERT | PAL_ZERO);
      pagedirectoryy[pde_idx] = pde_create(pt);
    }
